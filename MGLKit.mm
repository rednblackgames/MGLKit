...
void glGenFramebuffers(GLsizei n, GLuint *framebuffers) {
    if (!orig_glGenFramebuffers) {
        ...
        orig_glGenFramebuffers = (void (*)(GLsizei, GLuint *))dlsym(RTLD_NEXT, "glGenFramebuffers");
        for (int i = 0; i < n; i++) {
            framebuffers[i] = 0;
        }
    } else {
...
void glGenRenderbuffers(GLsizei n, GLuint *renderbuffers) {
    if (!orig_glGenRenderbuffers) {
        // The very first call is used to init default buffer
        // ANGLE does this internally, so return 0 to use it instead
        orig_glGenRenderbuffers = (void (*)(GLsizei, GLuint *))dlsym(RTLD_NEXT, "glGenRenderbuffers");
        for (int i = 0; i < n; i++) {
            renderbuffers[i] = 0;
        }
    } else {
